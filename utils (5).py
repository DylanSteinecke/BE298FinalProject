...
def get_disease_to_drug_to_proteins(drug_to_disease_to_proteins, drug_to_targets, your_disease_ids):
    diseases_to_drug_to_proteins = map_disease_to_drug_to_proteins(drug_to_disease_to_proteins)
    disease_to_drug_to_proteins = dict()
    for disease, values in diseases_to_drug_to_proteins.items():
        if disease in your_disease_ids:
            disease_to_drug_to_proteins[disease] = values
    return disease_to_drug_to_proteins
            
def get_disease_to_targets(disease_to_drug_to_proteins, drug_to_targets):
    disease_to_targets = dict()
    for disease, drugs in disease_to_drug_to_proteins.items():
        drug_targets = list()
        for drug in drugs:
            try: drug_targets += drug_to_targets[drug]
            except: continue
        disease_to_targets[disease] = drug_targets
    
    return disease_to_targets

def get_disease_to_pathway_protein_to_counts(disease_to_drug_to_proteins, disease_to_targets, disease_to_proteins):
    disease_to_pathway_protein_to_counts = dict()
    disease_to_disease_protein_to_counts = dict()
    disease_to_midpathway_protein_to_counts = dict()

    for disease, drug_to_pathway_proteins in disease_to_drug_to_proteins.items():
        disease_to_pathway_protein_to_counts[disease] = dict()
        disease_to_disease_protein_to_counts[disease] = dict()
        disease_to_midpathway_protein_to_counts[disease] = dict()
        targets = disease_to_targets[disease]
        try: 
            disease_proteins = disease_to_proteins[disease]
        except: 
            # Not correctly predicted drug mechanism
            continue
            
        for pathway_proteins in drug_to_pathway_proteins.values():
            for pathway_protein in pathway_proteins:
                disease_to_pathway_protein_to_counts[disease].setdefault(pathway_protein, 0)
                disease_to_pathway_protein_to_counts[disease][pathway_protein] += 1

                if pathway_protein not in disease_proteins:
                    if pathway_protein not in targets:
                        disease_to_midpathway_protein_to_counts[disease].setdefault(pathway_protein, 0)
                        disease_to_midpathway_protein_to_counts[disease][pathway_protein] += 1
                
                    #if pathway_protein not in targets and pathway_protein not in disease_proteins:
                    #    disease_to_mid_pathway_protein_to_counts[disease].setdefault(pathway_protein, 0)
                    #    disease_to_mid_pathway_protein_to_counts[disease][pathway_protein] += 1
                else:
                    disease_to_disease_protein_to_counts[disease].setdefault(pathway_protein, 0)
                    disease_to_disease_protein_to_counts[disease][pathway_protein] += 1
                
                    
    return disease_to_pathway_protein_to_counts, disease_to_disease_protein_to_counts, disease_to_midpathway_protein_to_counts
